...
def parser():
    ...
    return args.d, args.k, args.T, args.noise, args.xi, args.exp_name

def main():
    np.random.seed(0)

    d, k, T, noise_type, xi, exp_name = parser()
    ...
    gaps = [10**(-i) for i in np.linspace(1, 4, 6)]
    labels = [r'$10^{{-{}}}$'.format(i) for i in np.linspace(1, 4, 6)]
    ...
    Xi_hat = U.T @ Xi[0]
    bad_component = np.linalg.norm(Xi_hat[k:k+1, :], 2)      # u_{k+1} direction
    bulk_component = np.linalg.norm(Xi_hat[k+1:, :], 2)      # lambda_rest block
    print(f"Noise in u_{k+1} direction: {bad_component:.4e}")
    print(f"Noise in bulk direction: {bulk_component:.4e}")
    ...
    result_path = f"results/anpm_synthetic_gap_{exp_name}.csv"
